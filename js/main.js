...
app.HomeCatalogItemView = Backbone.View.extend({
  ...
  render: function() {
    this.$el.append(
      '<li style="list-style:none; text-align:center" class="catalog-item col-md-4"><img src="imgs/catalog/' +
        this.options.image +
        '" style="height:140px"><span class="catalog-item-text" style="text-align:center; display:block;">' +
        this.options.text +
        '<img class="catalog-arrow" src="imgs/catalog/catalog-arrow.png"></span></li>'
    );
    return false;
  }
});
...
app.HomeInterestItemView = Backbone.View.extend({
  ...
  render: function() {
    this.$el.append(
      '<li style="list-style:none; text-align:center" class="col-md-4">\
            <div class="interest-item">\
              <img class="interest-image" src="imgs/interests/' +
        this.options.image +
        '" style="">\
              <div class="interest-item-title">' +
        this.options.title +
        '</div>\
              <div class="interest-item-text" style="text-align:center; display:block;"><span>' +
        this.options.description +
        '</span></div>\
              <div class="interest-item-more"><a href="#item/' +
        this.options.slug +
        '">More Info</a><img class="catalog-arrow" src="imgs/catalog/catalog-arrow.png"></div>\
            </div>\
          </li>'
    );
    return false;
...
app.HomeView = Backbone.View.extend({
  ...
  render: function() {
    ...
    homeInterest.addItem({
      image: 'interest1.png',
      slug: 'jon_deere_410e',
      title: 'JOHN DEERE 410E',
      description:
        'Four wheel drive backhoe with extendahoe and lorum ipsum dolor amat sit quand donc quand gavant tiemps…'
    });
    homeInterest.addItem({
      image: 'interest2.png',
      slug: 'jon_deere_410e',
      title: 'CAT 308 EXCAVATOR',
      description:
        'The Cat 308 Mini Hydraulic Excavator delivers high performance with with the versatility of a swing boom…'
    });
    homeInterest.addItem({
      image: 'interest3.png',
      slug: 'jon_deere_410e',
      title: 'BOBCAT 331 COMPACT EXCAVATOR',
      description:
        'A mainstay in the Bobcat compact excavator lineup, the 331 exceeds expectations…'
    });
...
app.ContactView = Backbone.View.extend({
  ...
  render: function() {
    ...
    var marker = new google.maps.Marker({
      ...
      title: "Test's Rentals • Bobcat & Excavator Rentals"
    });

    var companyDetails = "<h3>Test's Rentals</h3>Bobcat & Excavator Rentals";
...
app.CategoryView = Backbone.View.extend({
  ...
  render: function() {
    ...
    var topTemplate =
      '<div class="category-row">' + this.options.title + '</div>';
    topTemplate +=
      '<div class="container category-sort"><div class="col-md-9">\
...
app.ItemView = Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    if (this.options.slug == 'jon_deere_410e') {
      ...
      this.options.description =
        'The Cat 308 Mini Hydraulic Excavator delivers high performance with the versatility of a swing boom front linkage in a durable Compact Radius design to help you work in the tightest applications. With the COMPASS display panel, Tier 4 Final engine and new hydraulic system, the 308 is more productive, versatile and cost effective!';
    }
  },
  render: function() {
    var template =
      '<div style="height:700px;" class="item_image">\
                        </div>\
                        ...
                        <span id="item-header">Starting At: $' +
      this.options.price +
      ' / Day</span></h2>\
    ...
    $('.item_image').css(
      'background-image',
      'url("imgs/item_images/' + this.options.image + '"'
    );

    var featuresTemplate =
      '<section class="item-page" id="item_features">\
          <div class="container">\
    ...
    var specsTemplate =
      '<section class="item-page" id="item_specs">\
        <div class="container">\
        <h2 style="text-align:center"><hr style="border-top:1px solid #d8d8d8">\
                        <span id="item-header">Tech Specs</span></h2>';

    specsTemplate +=
      '<div style="float:none; margin:0 auto" class="col-md-8"><table class="table table-striped">\
    <thead>\
      <tr>\
        <th>Overview:</th>\
        <th></th>\
...
app.Router = Backbone.Router.extend({
  ...
  showCategories: function(slug) {
    ...
    $('#interested').remove();
    $('#primary').prepend(
      '<section id="category"><div style="padding-top:50px" class="homepage-interests"></div></section">'
    );
    $('#primary').html(app.currentView.render().el);
  },
